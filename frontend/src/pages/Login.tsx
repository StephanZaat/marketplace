...
export default function Login() {
  ...
  const { containerRef: captchaRef, getResponse: getCaptchaResponse, reset: resetCaptcha } = useFriendlyCaptcha();
  ...
  const sendOtp = async (showToast = true) => {
    setLoading(true);
    try {
      const res = await api.post<{ otp_token: string; is_new_user: boolean }>("/auth/otp-send", {
        email,
        frc_captcha_response: getCaptchaResponse(),
      });
          ...
          {step === "email" && (
            <>
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                ...
                <div ref={captchaRef} className="frc-captcha" />
